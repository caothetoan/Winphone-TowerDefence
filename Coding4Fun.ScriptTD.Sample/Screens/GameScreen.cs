...
namespace Coding4Fun.ScriptTD.Sample.Screens
{
    public class GameScreen : IScreen
    {
        ...
        private readonly Vector2 _dragOffset = new Vector2(-50f);
        private Vector2 _drawOffset;
        private bool _canDrop;
        private Rectangle _testRect;
        private Vector2 _dragFinger;
        ...
        private void AssociateGuiCommands()
        {
            ...
            if (_gui.Controls.ContainsKey("speed1"))
                _gui.Controls["speed1"].Tapped += (c, v) => SetSpeed(_timeMult == 0 ? 1 : 2);
            if (_gui.Controls.ContainsKey("speed2"))
                _gui.Controls["speed2"].Tapped += (c, v) => SetSpeed(3);
            if (_gui.Controls.ContainsKey("speed3"))
                _gui.Controls["speed3"].Tapped += (c, v) => SetSpeed(1);
            ...
            else if (sender is Window)
            {
                if (_selectedTowerId != null)
                {
                    ...
                    if (_testRect.Contains((int)_dragPos.X, (int)_dragPos.Y))
                    {
                        ...
                        if (Game.Grid.IsCellValid(x, y) && Game.Grid[x, y].Buildable && Game.Grid[x, y].Tower == null)
                        {
                            _canDrop = true;
                        }
                        else
            ...
            if (_selectedTowerId != null && sender is Window)
            {
                if (_canDrop)
                {
                    ...
                    foreach (Path t in _testPaths)
                    {
                        ...
                        {
                            var p = t;
                            if (!Game.Paths.CalculateLandPath(ref p))
                            {
                                drop = false;
                                break;
                            }
        ...
        private void AddTowerButton(TowerInstance t, int x, int y)
        {
        ...
        public void Update(float elapsedSeconds)
        {
            ...
            if (touches.Count > 0)
            {
                if (_selectedTowerId == null)
                {
                    // Selecting tower (otherwise pass to _gui)
                    for (int i = 0; i < touches.Count; i++)
                    {
                        if (touches[i].State == TouchLocationState.Pressed)
                        {
                            var child = _gui.GetEnabledChildAtPoint(touches[i].Position);
                            if (child != null && child is TowerButton)
                            {
                                TowerDragging(child, touches[i].Position);
                                handled = true;
                                break;
                            }
                        }
                ...
                else
                {
                    ...
                    for (int i = 0; i < touches.Count; i++)
                    {
                        ...
                        {
                            TowerDragFinished(_gui, touches[i].Position);
                            handled = true;
                            break;
                        }
                        else if (touches[i].State == TouchLocationState.Moved)
                        {
                            TowerDragging(_gui, touches[i].Position);
                            handled = true;
                            break;
                        }
                    }
            ...
            while (!handled && TouchPanel.IsGestureAvailable)
            {
                var gesture = TouchPanel.ReadGesture();
                if (!_towerMenu.BubbleGesture(ref gesture))
            ...
            if (_towerMenu.Controls.ContainsKey("costDisplay"))
            {
                if (_selectedBtn != null)
                {
                    _towerMenu.Controls["costDisplay"].Value = _selectedBtn.UpgradeCost;
                    _towerMenu.Controls["costDisplay"].Visible = _selectedBtn.CanUpgrade;
                }
                else
                {
                    _towerMenu.Controls["costDisplay"].Visible = false;
                }
            }
            ...
            {
                _towerMenu.Controls["upgrade"].Enabled = _selectedBtn.CanUpgrade && (_selectedBtn.UpgradeCost <= Game.RemainingCash) && _selectedBtn.Instance.BuildProgress == 1;
            }
        ...
        public void OnNavigateTo()
        {
            ...
            else
            {
                ...
                {
                    int x, y;
                    Game.Grid.GetCell(Game.Towers[i].Position, out x, out y);
                    AddTowerButton(Game.Towers[i], x, y);
                }
            }
            if (_gui != null)
            {
                IVisual grid;
                if (_gui.NamedVisuals.TryGetValue("grid", out grid))
                {
                    grid.Visibility = Settings.ShowGrid ? Visibility.Enabled : Visibility.Disabled;
                }
            }
            SetSpeed(1);
            SetTowerButtons();
